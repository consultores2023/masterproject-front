    ...
    <form id="formulario" action="" @submit.prevent="validarFormulario">
      <div class="row">
        <div class="col col-12">
          ...
          <input v-model="nombre" @keyup.space="contador" type="text" class="form-control" id="inputNombre" placeholder="Nombre completo">
          <p>Validar nombre: <span class="text-success fw-bold"></span> {{nombre}}</p>
          <br>
          ...
          <p>Validar email: <span class="text-success fw-bold"></span></p>
          <br>
          <div class="row">
          ...
          <label for="selector"> Elije tu pais</label>
          <select v-model="pais" class="form-select" aria-label="Default select example">
            <option selected></option>
            <option value="colombia">Colombia</option>
            <option value="chile">Chile</option>
            <option value="argentina">Argentina</option>
          </select>
          <p>Validar país seleccionados:  <span class="text-success fw-bold"> {{pais}}</span></p>
          <br>
          <label for="areaComentarios" class="form-label text-start">Comentarios</label>
          <textarea v-model="comentarios" class="form-control" id="areaComentarios" rows="3"></textarea>
    ...
    validarFormulario(event) {
      ...
      if (this.nombre && this.contador>1 && this.edad && this.email) {
        ...
        setTimeout(() => {
          ...
          Object.assign.apply(this.$data, this.$options.data());
        })
        return true;
      }
      if (!this.nombre) {
